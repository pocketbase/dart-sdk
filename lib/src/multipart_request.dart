// -------------------------------------------------------------------
// This is a almost identical copy of http.MultipartRequest in order
// to add support for repeatable fields (aka. array values).
//
// See https://github.com/dart-lang/http/issues/277
// -------------------------------------------------------------------
...
// ignore_for_file: prefer_double_quotes, implementation_imports
...
class MultipartRequest extends BaseRequest {
  ...
  Stream<List<int>> _finalize(String boundary) async* {
    ...
    for (var field in fields.entries) {
      for (final v in field.value) {
        yield separator;
        yield utf8.encode(_headerForField(field.key, v));
        yield utf8.encode(v);
        yield line;
      }
    }
